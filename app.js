...
/* ---------- Checagem de versões descombinadas (app.js vs. index.html) ----------
   Se algum upload substituiu só parte dos arquivos, um app.js mais novo (ou mais
   velho) que o index.html pode tentar usar um elemento que não existe mais (ou
   ainda não existe). Isso falha do mesmo jeito de sempre — um TypeError críptico
   apontando pra uma linha de código — a única diferença é que, ao pegar isso aqui
   antes de qualquer outra coisa, a mensagem que aparece já vem explicando o que
   fazer, em vez de só o nome de uma variável interna. */
(function verificarVersaoConsistente() {
  const idsEsperados = ['btnOpcoes', 'modalOpcoes', 'btnAbrirNuvem', 'btnAbrirBackup', 'btnAbrirNotificacoes', 'fPagoComCartao'];
  const faltando = idsEsperados.filter((id) => !document.getElementById(id));
  if (faltando.length > 0) {
    throw new Error(
      'index.html e app.js parecem ser de versões diferentes (elemento(s) ausente(s): ' + faltando.join(', ') + '). ' +
      'Baixe o zip mais recente e substitua TODOS os arquivos de uma vez — não só alguns.'
    );
  }
})();
